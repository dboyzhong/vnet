...
// 2.002 360拿到的基准版本
// 2.003 优化cpu占用，减少不必要的内存复制
// 2.004 优化流控策略，显著提高启动速度
// 2.005 优化iOS底层实现，显著提高连接速度
// 2.006 支持ipv6 (iOS)
// 2.007 完善IUdxUnkownPackSink; 日志修改
...
{
public:
	virtual INT64 Addr2Int64(EXSOCKADDR* pAddr,UDP_LONG streamId) = 0;//将地址与一个WORD转化为一个64位常量
	virtual void TraceAddr(EXSOCKADDR* pAddr) = 0;//打印
	...
	virtual void TraceAddrEx(const char *tag, EXSOCKADDR* pAddr) = 0;
};
...
struct IUdxCfg									//单个UDX的一些设置
{
	...
	int enablemsgchannel;			//打开消息通道
	int enablewriteevent;			//打开写通知
	int enabletimer;				//打开定时器通知
}UDXPACKED;
...
struct IUdxGlobalCfg
{
	...
	IUdxLogSink* pLog;
	int bDisableLog;
	int udxlowcost;				//降低资源占用模式，仅用于客户端，大大减少线程数和资源占用。
}UDXPACKED;
...
class IUdxUnkownPackSink		// 应用层可以处理非UDX的包。可用于P2P或者其他协议
{
public:
	virtual void OnUnkownData(EXSOCKADDR * pAddr,BYTE* pData,long len){};
	virtual void OnThreadExit(){};

	// 因为UDX协议没有magic标志，如果只靠CRC16计算来判断是否Unknown
	// 所以OnUnkownData存在1/65535误判率
	// 为了解决这个问题新增回调函数，数据包先经过此函数处理
	// 如果函数返回TRUE则UDX不处理这个包，如果返回FALSE则UDX才处理
	// 要求外部的协议里有magic，可以简单精确判断是不是外部的私有协议
	virtual BOOL OnUnknownDataFilter(EXSOCKADDR * pAddr,BYTE* pData,long len){return FALSE;};

	// 50ms 计时器, 外界可以做重传
	virtual void OnUnknownTimer(){};
};
...
{
public:
	virtual void OnP2pConnect(EXSOCKADDR addrRemote,int errocode,char* user1,char* user2,INT64 dwuser){};//联接回调
};
...
public:
	...
	virtual EXSOCKADDR GetLocalAddr() = 0;
...
public:
	...
	virtual EXSOCKADDR *GetRemoteAddr() = 0;				//联接成功后，远端的IP信息
	virtual int __DSendUdxBuff(EXSOCKADDR * pAddr ,BYTE* pData,int len) = 0;		//可以用来发送非可靠数据
	virtual int __DSendUdxBuff2(EXSOCKADDR * pAddr ,BYTE* pData,int len) = 0;		//可以用来发送非可靠数据，但是会触发
	...
	virtual EXSOCKADDR *GetTransServerAddr() = 0;//返回当前走的中转IP
	...
	virtual int GetUncheckedUserBuffCount(int ich) = 0;
...
{
public:
	virtual void OnStreamPreConnect(EXSOCKADDR *pAddr,IUdxTcp * pTcp,int erro){};	//在此接口中，可设置各项参数
...
public:
	...
	virtual IUdxTcp* Connect(char* ip,UDP_SHORT port,BOOL bSync = FALSE,INT64 dwUser = 0,INT64 expectbew = 0,int lostrat = 50,
		char*strCName = NULL,IMultCardTcp* pMultCard = NULL,UdxConnectInfo*pInfo = NULL,BOOL bAutoRelase = TRUE) = 0;//通道名不得等于或超过25个字符
	...
	virtual EXSOCKADDR *GetLocalAddr() = 0;										//得到默认UDX端口占用信息
	virtual IUdxP2pClient* GetP2pClient() = 0;									//得到P2P接口
	virtual int __DSendUdxBuff(EXSOCKADDR * pAddr ,BYTE* pData,int len) = 0;		//直接向指定的地址发送非UDX 原始数据包
